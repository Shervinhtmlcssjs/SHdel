...
# -----------------------------------------------------------------------------
# SHdel - Single-file terminal app
# -----------------------------------------------------------------------------
...
COPYRIGHT = "©2025 Shervin Nosrati ©2025 - 2025 SHdel TM, ®Shervin. All rights reserved. France"
...
SERVERS_REGISTRY_NAME = "shdel_servers.json"   # stored in user's home directory
...
# =========================
# Node/NPM helpers (Options 12 & 13)
# =========================
def registry_path() -> Path:
    return Path.home() / SERVERS_REGISTRY_NAME


def npm_executable() -> Optional[str]:
    # Prefer Windows wrappers first
...
def run_command_stream(cmd: List[str], cwd: Path, save_output_to: Optional[Path] = None) -> int:
    ...
    If save_output_to is provided, appends the same output to that file.

    Windows note:
      - .cmd/.bat must be run via: cmd.exe /c <cmd...>
      - we also create a new process group so Ctrl+C can stop it.
    """
    log_f = None
    ...
    try:
        if save_output_to:
            ...
            log_f = save_output_to.open("a", encoding="utf-8")
            log_f.write("\n" + "=" * 80 + "\n")
            log_f.write(f"CMD: {' '.join(final_cmd)}\n")
            log_f.write(f"CWD: {cwd}\n")
            log_f.write(f"TIME: {datetime.now().isoformat(timespec='seconds')}\n")
            log_f.write("=" * 80 + "\n")
        ...
        print("Fix: ensure Node.js is installed and npm is in PATH.")
        print(f"Details: {e}")
        return 127
        ...
        if p is not None:
            try:
                if os.name == "nt":
                    # Requires CREATE_NEW_PROCESS_GROUP
...
def action_start_registered_server() -> None:
    ...
    if not confirm("Start dev server now? (npm run dev)"):
        print("CANCELLED")
        pause()
        return
